    ...
    switch (message.msgid) {
        case MachMsgIdConnect:
            if (message.sendPort != nil) {
                NSLog( @"mach server received connect message from port %d!",
            ...
            if (![message
                    sendBeforeDate:
                            [NSDate dateWithTimeIntervalSinceNow:
                                    1.0]]) {
                NSLog(@"failed to send message to %d, removing it from the clients!",
                        ((NSMachPort *)port).machPort);
                [removedPorts addObject:port];
            }
        } @catch (NSException *exception) {
            NSLog(@"failed to send message (exception) to %d, removing it from the clients!",
                    ((NSMachPort *)port).machPort);
            [removedPorts addObject:port];
        }
    }
